...
class FitnessEvaluator():
    ...
    def runCvodeSimulation(self, model, timeEnd, numberOfPoints):
        ...
        for i in range(model.numFloats + model.numBoundary):
            y0[i] = self.currentConfig['initialConditions'][i]
    ...
    def runSimulation(self, model, timeEnd, numberOfPoints):
        try:
            y = np.zeros(model.numFloats + model.numBoundary)
            t = np.linspace(0, timeEnd, numberOfPoints)
            for i in range(model.numFloats + model.numBoundary):
                y[i] = self.currentConfig['initialConditions'][i]
    ...
    def computeFitnessOfIndividual(self, index, model, objFunctionData):
        ...
        try:
            t, y = self.runCvodeSimulation(model, timeEnd, numberOfPoints)
